...
public class UnWarnCommand {
	...
	@Command(labels = "unwarn")
	@Permissible("lcore.command.unwarn")
	public void rankAddPerm(CommandSender sender, @Param("target") UUID uuid, @Param("reason {-p}") String reason) {
		...
		if (!targetProfile.hasActivePunishment(PunishmentType.WARN)) {
			sender.sendMessage(CC.translate("&cThat player is not warned."));
			return;
		}

		Punishment punishment = targetProfile.getActivePunishment(PunishmentType.WARN);
